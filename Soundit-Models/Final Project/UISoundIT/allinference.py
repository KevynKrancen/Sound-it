...
def Sentiment_recognition(video):
    ...
    filename = os.path.basename(video) # Extract the original filename from the video path
    output_file = f'{filename}_processed.mp4' # Use the original filename to create the output filename
    cap = cv2.VideoCapture(video)
