...
def Sentiment_recognition_cam(camNum):
    ...
    def inference(image):
        ...
        if detections:
            ...
            y1_new = max(0, y_center - new_height // 2) - int(height * 0.05)  # Shift the box up by 10% of the height
            y2_new = min(H, y_center + new_height // 2) - int(height * 0.05)  # Shift the box up by 10% of the height
            face = image[y1_new:y2_new, x1_new:x2_new]

            if face is not None:
                x = recognition_preprocessing([face])
                if x is not None:  # Add this check
        ...
        while True:
            ...
            if success:
                ...
                if faceemmotion == FaceEm[1]:
                    num_happy_faces += 1
                    print(num_happy_faces)
                if faceemmotion == FaceEm[0]:
                    num_angry_faces += 1
                    print(num_angry_faces)
                ...
                if len(sequence) == 30:
                    ...
                    cv2.putText(image, str(round(res[0], 2)), (int(image.shape[1] * 0.2), int(image.shape[0] * 0.08)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
                    cv2.putText(image, str(round(res[2], 2)), (int(image.shape[1] * 0.35), int(image.shape[0] * 0.08)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
                    cv2.putText(image, str(round(res[1], 2)), (int(image.shape[1] * 0.5), int(image.shape[0] * 0.08)), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1, cv2.LINE_AA)
